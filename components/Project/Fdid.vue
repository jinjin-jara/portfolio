...
<template>
    <div class="flex flex-col">
        ...
        <ProjectModalFunction>
            <li>index DB,
                <Tag :is-skill="true" text="dexie.js" />라이브러리를 활용하여 DID와 VC와 같이 복잡한 JSON 형태의 데이터를 저장 및 핸들링이 가능하도록 구현
                <ul class="my-2">
                    <div class="bg-zinc-200 px-3 py-2 rounded-lg">
                        <p class="mb-4 font-semibold">💡 로컬스토리지나 세션스토리지가 아닌 index DB를 사용한 이유</p>
                        <p class="break-words">DID와 VC와 같이 복잡한 JSON 형태의 데이터를 저장하고 관리하기 위해 IndexDB를 선택했습니다.<br />
                            Index DB는 로컬스토리지나 세션스토리지보다 구조화된 데이터 관리와 대량 데이터 처리에 유리합니다.</p>
                        <p class="mt-2 text-blue-500 cursor-pointer hover:text-blue-600 duration-300" @click="moveToWeb('https://jinnnkcoding.tistory.com/239')">관련 블로그 포스팅 보기</p>
                    </div>
                </ul>
            </li>
            <li>
                ...
                <ul class="list-circle pl-5">
                    <li>
                        브라우저에 저장된 DID 및 VC를 조회하고자 할 때 핀번호 입력 요구, 핀번호 불일치 시 조회 불가
                    </li>
                </ul>
            </li>
            <li>
                <Tag :is-skill="true" text="vue-qrcode" /> 라이브러리를 사용하여 VP JSON 데이터를 QR code로 생성,<br />QR reader를 통해 들어온
                VP 데이터
                파싱 후 검증 기능 구현
            </li>
        </ProjectModalFunction>
