...
const detectActiveTab = () => {
    ...
    sections.forEach((section) => {
        ...
        // Safari에서도 정확하게 확인
        if (offset >= scrollTop && offset < scrollTop + window.innerHeight / 2) {
...
<template>
    <div
        ...
        <div
            class="max-w-8xl px-2 lg:text-lg text-sm sm:text-sm grid grid-cols-5 gap-4 lg:gap-10 h-full items-center justify-center">
            ...
            <button :class="{ 'font-bold text-gray-800': activeTab === 'project' }" class="text-center"
                @click="scrollToSection('project')">Project</button>
            <button class="text-center" @click="() => { moveToWeb(`https://github.com/jinjin-jara`) }">Github</button>
            <button class="text-center" @click="() => { moveToWeb(`https://jinnnkcoding.tistory.com`) }">Tech
                Vlog</button>
        </div>
    </div>
</template>